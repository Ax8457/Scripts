...
*/
//noise info
const u8 handshake_name[19] = "test_handshake_name";
const u8 identifier_name[14] = "Axel_Biegalski";
...
*/
//es
void mix_dh(u8 chaining_key[NOISE_HASH_LEN], u8 key[NOISE_SYMMETRIC_KEY_LEN], const u8 private_key[NOISE_PUBLIC_KEY_LEN], const u8 public_key[NOISE_PUBLIC_KEY_LEN])
{
	u8 dh_calculation[NOISE_PUBLIC_KEY_LEN];
	//diffie hellman share secret
    curve25519(dh_calculation, private_key, public_key);
	kdf(
        chaining_key, //C2
        key, //k1
        NULL, 
...
//server side
void message_decrypt(u8 *dst_plaintext, const u8 *src_ciphertext, size_t src_len, u8 key[NOISE_SYMMETRIC_KEY_LEN], u8 hash[NOISE_HASH_LEN])
{
...
//ee
void message_ee(const u8 ephemeral_public[NOISE_PUBLIC_KEY_LEN], const u8 ephemeral_private[NOISE_PUBLIC_KEY_LEN] ,u8 chaining_key[NOISE_HASH_LEN]){
	//C5
	u8 dh_calculation[NOISE_PUBLIC_KEY_LEN];
	//diffie hellman share secret
	curve25519(dh_calculation, ephemeral_private, ephemeral_public);
    kdf(
        chaining_key, //C2
        NULL, //k1
        NULL, 
...
//se
void message_se(const u8 ephemeral_private_key[NOISE_PUBLIC_KEY_LEN], const u8 remote_static[NOISE_PUBLIC_KEY_LEN] ,u8 chaining_key[NOISE_HASH_LEN]){
	//C1
	u8 dh_calculation[NOISE_PUBLIC_KEY_LEN];
	//diffie hellman share secret
	curve25519(dh_calculation, ephemeral_private_key, remote_static);
	kdf(
        chaining_key, //C2
        NULL, //k1
        NULL, 
